...
const Question7_2_3 = ({route,navigation}) =>{
  const [answer,setAnswer] = useState('')
  const {nickname} = route.params;
  const qid = "sub_question7_2_3"
  const solved = true;
    return(
      ...
      <View style={styles.inputView}>
        <Text style={{fontWeight:"bold"}}>
        Ok, so you think that he can rent it for [x] days. How much would Jim pay in total?</Text>
        <TextInput style={styles.input} multiline={true} value={answer} onChangeText={setAnswer}/>
        <TouchableOpacity onPress={()=>{
          updateDB(nickname,qid,answer)
        navigation.navigate("SelectStrategy7",{nickname:nickname, solved1:solved, solved2:solved})
        }}>

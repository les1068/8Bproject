...
const Question4_1_3 = ({route,navigation}) =>{
  const [answer,setAnswer] = useState('')
  const {nickname} = route.params;
  const qid = "sub_question4_1_3"
    return(
      ...
      <View style={styles.inputView}>
        <Text style={{fontWeight:"bold"}}>
        Nice work! Now, what do Elena’s, Karla’s and Faye’s scores add up to?</Text>
        <TextInput style={styles.input} multiline={true} value={answer} onChangeText={setAnswer}/>
        <TouchableOpacity onPress={()=>{
        updateDB(nickname,qid,answer) 
        navigation.navigate("Question4_1_4",{nickname:nickname})
        }}>
